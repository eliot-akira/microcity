...
/**
 * Unused but kept for reference - Generates "minimap"
 * ---
...
function SplashCanvas(parentID, tileSet, id) {
  ...
  if (!(this instanceof SplashCanvas)) { return new SplashCanvas(parentNode, tileSet, id) }
  ...
  const height = SplashCanvas.DEFAULT_HEIGHT
  const width = SplashCanvas.DEFAULT_WIDTH
  ...
  if (existing !== null) {
    if (existing.parentNode === parentNode) {
      console.warn(
        'There was already an object with the same ID as SplashCanvas - replacing it!'
      )
      parentNode.replaceChild(this._canvas, existing)
    } else {
      console.warn(
        'SplashCanvas id ' + id + ' already exists somewhere in document'
      )
